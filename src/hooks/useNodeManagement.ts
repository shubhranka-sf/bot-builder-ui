...
import { getId, mockAvailableFunctions } from '../data/mockData'; // Assuming functions/getId are here
...
interface NodeManagementProps {
    ...
    setSelectedNode: React.Dispatch<React.SetStateAction<Node | null>>; // To update selected node state after updates
}

/**
 * Hook for managing node data updates and additions.
 */
export function useNodeManagement({
    ...
    const updateNode = useCallback(
        <T extends StartNodeData | IntentNodeData | ActionNodeData | FormNodeData>(
            ...
            // Update the selected node state if the updated node is the selected one
             if (updatedNode) {
                 setSelectedNode(prev =>
                     prev && prev.id === nodeId && prev.type === nodeType ? updatedNode : prev
                 );
             }
        },
        [setNodes, setSelectedNode] // Added setSelectedNode dependency
    );
    ...
    const updateIntentNode = useCallback(
        (nodeId: string, newIntentId: string, newExamples?: string[]) => {
            ...
            // If examples were explicitly provided (meaning they were edited), update the global definition
            if (newExamples !== undefined) {
                setIntents((prevIntents) => {
                    ...
                    if (intentIndex > -1) {
                        ...
                        console.log('Updated intent definition:', updatedIntents[intentIndex]);
                        return updatedIntents;
                    } else {
                        console.warn(`Intent definition "${newIntentId}" not found during example update.`);
                        // Optionally add it as a new definition if desired
                        // return [...prevIntents, { id: newIntentId, label: newIntentId, examples: finalExamples, entities: parsedEntities }];
                    }
                    return prevIntents;
                });
            }
    ...
    const updateFormNode = useCallback(
        (nodeId: string, formData: Partial<FormNodeData>) => {
            ...
            // Form definitions are self-contained within the node data in this structure
        },
        [updateNode]
    );
     ...
     const getCenterPosition = useCallback((): XYPosition => {
        ...
        if (flowPane) {
            ...
            // Adjust offsets slightly for better centering feel
            const xOffset = 100; // Half of typical node width
            const yOffset = 50; // Roughly half of typical node height
            return screenToFlowPosition({ x: bounds.width / 2 - xOffset, y: bounds.height / 3 - yOffset });
        }
         // Fallback if pane dimensions aren't available
         const nodesCount = getNodes().length;
         return { x: 250 + (nodesCount % 5) * 50, y: 150 + Math.floor(nodesCount / 5) * 50 };
    }, [screenToFlowPosition, getNodes]); // Added getNodes dependency
    ...
    const handleAddNode = useCallback(
        (type: 'intent' | 'action' | 'end' | 'start' | 'form') => {
            ...
            if (type === 'start') {
                ...
                newNodeData = { name: 'New Form', formId: `form_${getId().slice(-4)}`, slots: [] };
            } else if (type === 'end') {
                newNodeData = {}; // No data needed for end node
            }

            const newNode: Node = { id: getId(), type, position, data: newNodeData };
            addNodes(newNode);
        },
        [addNodes, getCenterPosition, definedActions, intents] // Keep dependencies
    );
        ...
        (newAction: ActionDefinition) => {
            setDefinedActions((prev) => {
                ...
                const completeAction: ActionDefinition = {
                 ...
                 if (completeAction.valueType === 'text') delete completeAction.value; // Optional cleanup
                 if (completeAction.valueType === 'function') delete completeAction.variations; // Optional cleanup
