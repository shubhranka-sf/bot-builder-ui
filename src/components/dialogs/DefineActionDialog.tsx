...
import { mockAvailableFunctions } from '../../data/mockData'; // Or pass as prop
...
const DefineActionDialog: React.FC<DefineActionDialogProps> = ({
    ...
    onSubmit,
    existingActionNames,
    availableFunctions = mockAvailableFunctions // Default to imported mock data
}) => {
    ...
    const [newActionFunctionValue, setNewActionFunctionValue] = useState<string>(availableFunctions[0]?.name || ''); // Default to first function

    const handleValueTypeToggle = useCallback((type: ValueInputType) => {
        setNewActionValueType(type);
        if (type === 'text') {
            setNewActionFunctionValue(availableFunctions[0]?.name || ''); // Reset function value maybe? Or keep last? Resetting seems safer.
             if (newActionVariations.length === 0) setNewActionVariations(['']); // Ensure one input
        } else {
            // Don't clear variations immediately, user might toggle back
        }
    }, [availableFunctions]); // Add dependency

    const handleVariationChange = useCallback((index: number, value: string) => {
        setNewActionVariations(prev => { const copy = [...prev]; copy[index] = value; return copy; });
    }, []);
    const handleAddVariationInput = useCallback(() => setNewActionVariations(prev => [...prev, '']), []);
    ...
    const handleSubmit = useCallback(() => {
        ...
        if (existingActionNames.includes(name)) { toast.error(`Action Name "${name}" already exists.`); return; }
        ...
        if (newActionValueType === 'text') {
            ...
            const functionValue = newActionFunctionValue.trim();
             if (!functionValue) { toast.error('Function name cannot be empty.'); return; }
              if (!availableFunctions.some(f => f.name === functionValue)) {
                 toast.error(`Selected function "${functionValue}" is not valid.`); return;
             }
        ...
        // Reset form and close
         ...
         setNewActionVariations(['']); setNewActionFunctionValue(availableFunctions[0]?.name || '');
         onClose();
    }, [
        newActionTitle, newActionName, newActionValueType, newActionVariations, newActionFunctionValue,
        existingActionNames, availableFunctions, onSubmit, onClose
    ]);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg relative max-h-[90vh] flex flex-col">
                ...
                <div className="space-y-4 overflow-y-auto pr-2 flex-grow scrollbar-thin">
                    {/* Title Input */}
                    <div>
                        <label htmlFor="newActionTitle" className="block text-sm font-medium text-gray-700 mb-1"> Title <span className="text-gray-400 text-xs">(optional)</span> </label>
                        <input type="text" id="newActionTitle" value={newActionTitle} onChange={(e) => setNewActionTitle(e.target.value)} className="w-full input input-bordered input-sm" placeholder="Display title" />
                    </div>
                    {/* Name Input */}
                    <div>
                        <label htmlFor="newActionName" className="block text-sm font-medium text-gray-700 mb-1"> Name (ID) <span className="text-red-500">*</span> </label>
                        <input type="text" id="newActionName" value={newActionName} onChange={(e) => setNewActionName(e.target.value)} required className="w-full input input-bordered input-sm font-mono" placeholder="e.g., action_lookup or utter_greet" />
                    ...
                    <div>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => handleValueTypeToggle('text')} className={`btn-sm btn-toggle ${newActionValueType === 'text' ? 'active' : ''}`}> <Type size={14} /> Text/Variations </button>
                            <button onClick={() => handleValueTypeToggle('function')} className={`btn-sm btn-toggle ${newActionValueType === 'function' ? 'active' : ''}`}> <GitBranch size={14} /> Function </button>
                        </div>
                    </div>
                    ...
                    {newActionValueType === 'text' ? (
                            ...
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-1 scrollbar-thin border border-gray-200 bg-white p-2 rounded">
                                {newActionVariations.map((variation, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        <textarea value={variation} onChange={(e) => handleVariationChange(index, e.target.value)} className="flex-grow textarea textarea-bordered textarea-xs" placeholder={`Variation ${index + 1}`} rows={2} />
                                        <button onClick={() => handleRemoveVariationInput(index)} type="button" disabled={newActionVariations.length <= 1} className={`btn btn-ghost btn-xs p-1 text-red-500 disabled:text-gray-400 ${newActionVariations.length <= 1 ? '' : 'hover:bg-red-100'}`} title="Remove"> <Trash2 size={16} /> </button>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1 h-4 truncate" title={availableFunctions.find(f => f.name === newActionFunctionValue)?.description}> {availableFunctions.find(f => f.name === newActionFunctionValue)?.description} </p>
                        </div>
                    )}
                </div>
                {/* Footer Buttons */}
                <div className="flex justify-end gap-3 border-t pt-4 mt-4 flex-shrink-0">
                    ...
                    <button onClick={handleSubmit} className="btn btn-primary btn-sm"> <Check size={16} /> Create Action </button>
                </div>
            </div>
        </div>
    );
