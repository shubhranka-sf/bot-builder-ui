  ...
  const exportFlowData = useCallback(async () => {
    ...
    const formattedActions = definedActions.map((action) => {
      ...
      try {
        ...
        const isButton = Array.isArray(parsed) &&
    ...
    startNodes.forEach((startNode) => {
      ...
      const steps = path
        .filter((id) => nodeMap[id]?.type !== 'start' && nodeMap[id]?.type !== 'end')
        .map((id) => {
    ...
    const currentVersion = parseInt(localStorage.getItem(versionCountKey) || '0');
