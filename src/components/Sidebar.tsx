...
const intents: IntentDefinition[] = [
  { id: "intent_greet", label: "Greeting", examples: ["hello", "hi", "hey"] },
  {
    id: "intent_order",
    label: "Place Order",
    examples: ["order", "buy", "purchase"],
  },
  {
    id: "intent_support",
    label: "Request Support",
    examples: ["support", "help", "assist"],
  },
  {
    id: "intent_goodbye",
    label: "Goodbye",
  ...
  },
  {
    ...
    label: "FAQ - Shipping",
    examples: ["shipping", "delivery", "ship"],
  },
  {
    id: "intent_faq_returns",
    label: "FAQ - Returns",
    examples: ["return", "exchange", "refund"],
  },
];
...
interface SidebarProps {
  ...
  onUpdateAction: (
    nodeId: string,
    actionData: Omit<ActionDefinition, "id">
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [currentValueInputType, setCurrentValueInputType] =
  ...
  const [newActionValueType, setNewActionValueType] =
    useState<ValueInputType>("text");
  ...
  const handleConfigChange = (
    field: keyof Omit<ActionDefinition, "id">,
    value: string | ValueInputType
  ) => {
  ...
  const handleValueTypeToggle = (type: ValueInputType) => {
    ...
    const newValue =
      type === "function" ? availableFunctions[0]?.name || "" : "";
  ...
  const handleIntentDialogSubmit = () => {
    const examplesArray = exampleInputs
      .map((example) => example.trim())
      .filter((example) => example !== "");
    console.log("New intent definition:", {
      name: newIntentName,
  ...
  const handleActionDialogSubmit = () => {
    const finalValue =
      newActionValueType === "function" ? newActionValue : newActionValue;
  ...
  const currentIntentLabel =
    intents.find((i) => i.id === selectedNode.data.intentId)?.label ||
    selectedNode.data.intentId ||
    "N/A";
  const currentActionTitle =
    selectedNode.data.title || selectedNode.data.name || "N/A";
    ...
    <div className="relative w-72 bg-white border-l border-gray-200 shadow-lg flex flex-col h-full transition-all duration-300 ease-in-out z-10">
      ...
      <div className="flex border-b border-gray-200">
        {isIntentNodeSelected && (
          <div
            className={`flex-1 py-2 px-4 text-sm font-medium text-center flex items-center justify-center gap-1 border-b-2 border-blue-500 text-blue-600`}
          >
            <Bot size={16} /> Intent
          </div>
        )}
        {isActionNodeSelected && (
          <div
            className={`flex-1 py-2 px-4 text-sm font-medium text-center flex items-center justify-center gap-1 border-b-2 border-green-500 text-green-600`}
          >
            <Zap size={16} /> Action
          </div>
        )}
            ...
            {isIntentNodeSelected && (
              <>
                ...
                <p className="text-sm">
                  ...
                  <code className="text-xs bg-blue-100 text-blue-700 px-1 py-0.5 rounded">
                    {selectedNode.data.intentId || "N/A"}
                  </code>
                </p>
              </>
            )}
              ...
              <>
                ...
                <div className="text-sm flex items-center gap-2">
                  <span className="text-gray-500">
                    {currentActionValueType === "function"
                      ? "Function:"
                      : "Value:"}
                  </span>
                  {currentActionValueType === "function" && (
        ...
        {mode === "edit" && isIntentNodeSelected && (
          <div>
            ...
            <p className="text-xs text-gray-500 mb-3">
              Edit the intent assigned to this node.
            </p>
            <div>
              <label
                className="block text-sm font-medium text-gray-700 mb-1"
              ...
              {intents
                ...
                ?.examples?.map((example, index) => (
                  <input
                    ...
                    value={example}
                    onChange={(e) =>
                      handleInputExampleChange(index, e.target.value)
                    }
                    className="block w-full border border-gray-300 rounded-md p-2 focus:ring-gray-500 focus:border-gray-500 shadow-sm text-sm"
                  />
                ))}
          ...
          <>
            ...
            {isIntentNodeSelected && (
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Select an intent to assign to this node.
                </p>
                <ul className="space-y-1 max-h-96 overflow-y-auto border rounded p-1">
                  {intents.map((intent) => (
                    <li key={intent.id}>
                      ...
                      >
                        {intent.label} (
                        <code className="text-xs">{intent.id}</code>)
                      </button>
                    </li>
                  ))}
                </ul>
            ...
            {isActionNodeSelected && (
              <div>
                ...
                <p className="text-xs text-gray-500 mb-3">
                  Select a predefined action type to replace the current node's
                  configuration.
                </p>
                <ul className="space-y-1 max-h-96 overflow-y-auto border rounded p-1">
                    ...
                    <li key={`${action.name}-${index}`}>
                      ...
                      >
                        ...
                        {action.valueType === "function" && (
                          <GitBranch size={14} className="text-gray-500 ml-2" />
                        )}
                      </button>
                    </li>
                  ))}
          ...
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md relative max-h-[80vh] flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4">
              ...
              <div>
                ...
                {exampleInputs.map((example, index) => (
                  <div key={index} className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      className="block w-full border border-gray-300 rounded-md p-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm text-sm"
                      placeholder={`Example ${index + 1} (e.g., "I want a pizza")`}
                    />
                    {index === exampleInputs.length - 1 && (
                      <button
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Provide examples of how a user might express this intent.
                </p>
              </div>
            </div>
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-200 mt-4">
              <button
                onClick={() => setShowIntentDialog(false)}
                className="px-4 py-2 text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md font-medium"
              >
                Cancel
              </button>
              <button
            ...
            <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="block w-full border border-gray-300 rounded-md p-2 focus:ring-green-500 focus:border-green-500 shadow-sm text-sm"
                  placeholder="User-friendly title (e.g., Send Confirmation)"
                />
                <p className="text-xs text-gray-500 mt-1">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1 rounded border text-sm ${newActionValueType === "text" ? "bg-green-100 border-green-300 text-green-800" : "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100"}`}
                  >
                    <Type size={14} /> Text
                  </button>
                  <button
                    onClick={() => handleNewActionValueTypeToggle("function")}
                    className={`flex items-center gap-1 px-3 py-1 rounded border text-sm ${newActionValueType === "function" ? "bg-gray-100 border-gray-300 text-gray-800" : "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100"}`}
                  >
                    <GitBranch size={14} /> Function
                  </button>{" "}
                  {/* Updated Icon */}
              ...
              {newActionValueType === "text" ? (
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    The static value or configuration for this action.
                  </p>
                </div>
              ) : (
                <div>
                  <label
                    className="block text-sm font-medium text-gray-700 mb-1"
                    htmlFor="newActionValueFunction"
                  >
                    Select Function
                  </label>
                  <select
                    id="newActionValueFunction"
                    value={newActionValue}
                    onChange={(e) => setNewActionValue(e.target.value)}
                    className="block w-full border border-gray-300 rounded-md p-2 focus:ring-gray-500 focus:border-gray-500 shadow-sm text-sm"
                  >
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Select the function this action should execute.
                  </p>
                </div>
              )}
            </div>
